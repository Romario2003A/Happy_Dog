...
function generateClinicalHistoryPdf() {
  ...
  const consultation = (filled = false) => `<table class="consult"><tr><th>FECHA</th><th>PESO</th><th>FC</th><th>FR</th><th>T°</th></tr><tr><td>${filled ? escapeHtml(formatShortDate()) : ''}</td><td>${filled ? escapeHtml(form.value.weightKg || pet.weightKg || '') : ''}</td><td>${filled ? escapeHtml(form.value.fc || '') : ''}</td><td>${filled ? escapeHtml(form.value.fr || '') : ''}</td><td>${filled ? escapeHtml(form.value.temperatureC || '') : ''}</td></tr><tr><th colspan="2">Anamnesis y exploración física</th><td colspan="3" class="text">${filled ? escapeHtml(form.value.anamnesis || '') : ''}</td></tr><tr><th colspan="2">Exámenes complementarios</th><td colspan="3" class="text">${filled ? escapeHtml([selectedExams, form.value.examOther].filter(Boolean).join(', ')) : ''}</td></tr><tr><th>Dx. P-P</th><td colspan="2" class="text">${filled ? escapeHtml(form.value.presumptiveDx || '') : ''}</td><th>Dx. P-D</th><td class="text">${filled ? escapeHtml(form.value.definitiveDx || '') : ''}</td></tr><tr><th colspan="2">Tratamiento</th><td colspan="3" class="text">${filled ? escapeHtml(form.value.treatment || '') : ''}</td></tr><tr><th colspan="2">Frecuencia</th><td colspan="3" class="text">${filled ? escapeHtml(form.value.frequency || '') : ''}</td></tr></table>`;
